...
local ui_urls = {
    "https://raw.githubusercontent.com/hdanhhub/UI/refs/heads/main/themevtd_VTDRoblox.lua",
}
...
for i, url in ipairs(ui_urls) do
    ...
    if success and result then
        ...
        local scaleX = math.max(screenSize.X * 0.72, 820)
        local scaleY = math.max(screenSize.Y * 0.80, 600)

        v466 = result:MakeWindow({
            Title = "VTD Roblox",
            SubTitle = "by HDANH | NOPERMC",
            SaveFolder = "Redz | redz lib v5.lua",
            Size = UDim2.new(0, scaleX, 0, scaleY),
            MinSize = Vector2.new(820, 600)
        })
        ui_loaded = true
        print("✅ UI loaded from URL " .. i)
        -- Inject UIScale vào tất cả ScreenGui để tự scale theo màn hình
        task.delay(0.5, function()
            local camVP = workspace.CurrentCamera.ViewportSize
            local baseW = 1366
            local scFactor = math.min(camVP.X / baseW, camVP.Y / 768) * 1.15
            scFactor = math.max(scFactor, 1.0)
            for _, gui in ipairs(game:GetService("CoreGui"):GetDescendants()) do
                if gui:IsA("ScreenGui") and not gui:FindFirstChildOfClass("UIScale") then
...
if not ui_loaded then
    ...
    sg.Name = "HDANHBackupUI"
    sg.Parent = game:GetService("CoreGui")
    sg.ResetOnSpawn = false
    -- Bật AutoScale toàn màn hình
    sg.IgnoreGuiInset = true

    local screenSz = workspace.CurrentCamera.ViewportSize
    local bkW = math.max(screenSz.X * 0.75, 700)
    local bkH = math.max(screenSz.Y * 0.75, 480)
    local bkOffX = -(bkW / 2)
    local bkOffY = -(bkH / 2)

    local main = Instance.new("Frame")
    main.Name = "MainFrame"
    main.Size = UDim2.new(0, bkW, 0, bkH)
    main.Position = UDim2.new(0.5, bkOffX, 0.5, bkOffY)
    ...
    title.Text = "🌸 VTD Roblox - Blox Fruits VIP"
    ...
    infoText.Size = UDim2.new(1, -30, 1, -80)
    infoText.Position = UDim2.new(0, 15, 0, 70)
...
✅ Giải pháp:
  ...
  4. Liên hệ HDANH | NOPERMC để hỗ trợ
...
l_ImageButton_0.Image = "rbxassetid://82433582607748"
...
local v484 = v466:MakeTab({"Th\195\180ng Tin | Info","rbxassetid://82433582607748"})
local v485 = v466:MakeTab({"Farming","rbxassetid://82433582607748"})
local v486 = v466:MakeTab({"Auto Fishing","rbxassetid://82433582607748"})
local v487 = v466:MakeTab({"Quest | Items","rbxassetid://82433582607748"})
local v488 = v466:MakeTab({"Volcano Dojo","rbxassetid://82433582607748"})
local v489 = v466:MakeTab({"Sea Event","rbxassetid://82433582607748"})
local v490 = v466:MakeTab({"Race V4","rbxassetid://82433582607748"})
local v491 = v466:MakeTab({"Raid Fruits","rbxassetid://82433582607748"})
local v492 = v466:MakeTab({"Fruits | Check Stock","rbxassetid://82433582607748"})
local v493 = v466:MakeTab({"Teleport","rbxassetid://82433582607748"})
local v494 = v466:MakeTab({"PvP,Player","rbxassetid://82433582607748"})
local v495 = v466:MakeTab({"Shop","rbxassetid://82433582607748"})
local v496 = v466:MakeTab({"Settings","rbxassetid://82433582607748"})
v484:AddDiscordInvite({
    Name = "Tham Gia Discord",
    Description = "Link Sever Discord",
    Logo = "rbxassetid://82433582607748",
    Invite = " https://discord.gg/Utg3Vxp3b5"
})
